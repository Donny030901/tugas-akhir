...
class PembelianDetailController extends Controller
{
    ...
    public function data($id)
    {
        ...
        foreach ($detail as  $item) {
            ...
            $row['jumlah']      = '<input type="number" class="form-control input-sm edit-jumlah" data-id="' . $item->id_pembelian_detail . '" value="' . $item->jumlah . '">';
    ...
    public function store(Request $request)
    {
        ...
        $cek_pembelian_detail = PembelianDetail::where($where)->first();
        if ($cek_pembelian_detail != null) {
            $detail = PembelianDetail::find($cek_pembelian_detail['id_pembelian_detail']);
            $detail->jumlah = $cek_pembelian_detail['jumlah'] + 1;
            $detail->subtotal = $produk->harga_beli * $detail->jumlah;
            $detail->update();
        } else {
